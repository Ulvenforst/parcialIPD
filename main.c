...
/*     } */
/* } */
// Versión paralelizada del filtro Sobel
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height) {
    const int Gx[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
    const int Gy[3][3] = {{-1, -2, -1}, {0, 0, 0}, {1, 2, 1}};

    // Paralelizar los bucles externos
    #pragma omp parallel
    {
        ...
        for (int y = 1; y < height - 1; y++) {
            for (int x = 1; x < width - 1; x++) {
                ...
                // Aplicar máscaras de Sobel
                for (int ky = -1; ky <= 1; ky++) {
                    for (int kx = -1; kx <= 1; kx++) {
                        const int idx = (y + ky) * width + (x + kx);
                        sumX += imagen[idx] * Gx[ky + 1][kx + 1];
                        sumY += imagen[idx] * Gy[ky + 1][kx + 1];
                    }
                }

                // Calcular magnitud del gradiente
                const int magnitude = abs(sumX) + abs(sumY);
